...
export function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <article className={`project-card ${isMajor ? 'is-major' : ''}`} data-project={`0${index + 1}`}>
      <Link className="project-visual-link" href={`/projects/${project.slug}`} aria-label={`View ${project.shortTitle} case study`} data-cursor="focus" data-sound="project">
      ...
      <div className="project-content">
        ...
        <h3><Link href={`/projects/${project.slug}`} data-sound="project">{project.shortTitle}</Link></h3>
        {isMajor && (
        ...
        <div className="project-actions">
          <Link className="project-action project-action-primary" href={`/projects/${project.slug}`} data-sound="project">
          ...
          {!project.githubUrl && (
            <Link className="project-action project-action-secondary" href={`/projects/${project.slug}`} data-sound="project">
